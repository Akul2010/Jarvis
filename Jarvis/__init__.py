...
try:
    ...
    import features.jarvis_api.jarvis_api.JarvisAPI
    ...
    import features.internet_speed_test.internet_speed_test.speed_test
    import features.whatsapp_message.whatsapp_message.send_whatsapp_message
    import features.youtube_play.youtube_play.yt_play
    import features.youtube_video_downloader.youtube_video_downloader.download_yt_video
    # import features.utilities.utilities

except Exception as e:
    from Jarvis.features.weather import weather as wea
    from Jarvis.features.website_open import website_open
    from Jarvis.features.send_mail import send_mail
    from Jarvis.features.date_time import date_time
    from Jarvis.features.launch_app import launch_app
    from Jarvis.features.news import news as nw
    from Jarvis.features.tell_me_about import tell_me_about as tma
    from Jarvis.features.face_recognition import dataset_create as dc
    from Jarvis.features.face_recognition import train as train
    from Jarvis.features.face_recognition import predict as predict
    from Jarvis.features.face_recognition import face_reco as fr
    from Jarvis.features.show_me_my_images import show_me_my_images as my_photos
    from Jarvis.features.setup import setup as setup_assistant
    from Jarvis.features.google_photos import google_photos as gp
    from Jarvis.features.joke import joke
    from Jarvis.features.hot_word_detection import hot_word_detection as wake_word
    from Jarvis.features.mic_input_ai.mic_input_ai import SpeechRecognition as SpeechRecognitionAI
    from Jarvis.features.jarvis_api.jarvis_api import JarvisAPI
    from Jarvis.features.detection.detection import Detections
    from Jarvis.features.chatbot.chatbot import start_chatbot_large as chatbot_lrg
    from Jarvis.features.chatbot.chatbot import start_chatbot_small as chatbot_sml
    from Jarvis.features.chatbot.chatbot import load_chatbot_models as load_ch_model
    from Jarvis.features.chatbot.todo import make_todo, read_notes_if_exist, delete_todo
    from Jarvis.features.chatbot.jarvis_ai_neural_engine import neural_engine
    from Jarvis.features.internet_speed_test.internet_speed_test import speed_test
    from features.whatsapp_message.whatsapp_message import send_whatsapp_message
    from features.youtube_play.youtube_play import yt_play
    from features.youtube_video_downloader.youtube_video_downloader import download_yt_video
    # from Jarvis.features.utilities import utilities
...
class JarvisAssistant:
    def __init__(self, sync=True, token=None, disable_msg=False, load_chatbot_model=True,
        ...
        if high_accuracy_chatbot_model_with_neural_engine:
            self.Jarvis_neural_engine_obj = neural_engine.JarvisNeuralEngine()
        else:
            self.Jarvis_neural_engine_obj = None
        ...
        if token is None or not sync:
            if not disable_msg:
                print("\n")
                cprint(
                    "Note: This library is based on no longer in development."
                    "Check Out Lightweight (More Powerful) Version of Jarvis- \n"
                    "AdonisAI: https://pypi.org/project/AdonisAI \n\n", color='red')
                cprint("Set 'obj = Jarvis.JarvisAssistant(sync=False)' if you do not want to use API",
                       color='green')
                ...
                cprint("Its free to use. Even API services is also free. \n"
                       "Well, Jarvis need support of your to keep this project and it's API alive.\n"
                       "So, your contribution/donation will be appreciated. \n"
                       , color='blue')
                cprint("DONATE: https://www.buymeacoffee.com/dipeshpal", color='red')
                cprint(
                    "Set 'obj = Jarvis.JarvisAssistant(disable_msg=True)' if you do not want to display this message",
                    color='green')
                cprint("Initializing Jarvis...", color='yellow')
        ...
        if not self.sync:
            ...
            if self.token is None:
                cprint("TOKEN NOT FOUND",
                       color='yellow')
                cprint("Set 'obj = Jarvis.JarvisAssistant(sync=true, token='xyz')' if you want to use this API",
                       color='red')
                cprint("Obtain your token from: http://jarvis-ai-api.herokuapp.com/", color='green')
                ...
                status, res = self.Jarvis_api.get_user_data(self.token)
                if not res['status']:
    ...
    def text2speech(self, text, lang='en'):
        ...
        You can set backend of tts while creating object of Jarvis class. Default is PYTTSX3.
        :param text: str
            text (String)
        :param lang: str
            default 'en'
        ...
        if self.backend_tts_api == 'gtts':
            ...
            try:
                voice_file_name = "configs/Jarvis-Voice.txt"
                if not os.path.exists(voice_file_name):
                    cprint("You can try different voices. This is one time setup. You can reset your voice by deleting"
                           "'configs/Jarvis-Voice.txt' file in your working directory.",
                           color='blue')
                ...
                print("Error occurred while creating config file for voices in pyttsx3 in 'text2speech'.",
                      "Contact maintainer/developer of Jarvis")
            try:
    ...
    def set_user_data(self):
        ...
        :return: None
        """
        self.Jarvis_api.set_user_data()

    def Jarvis_configure_hand_detector(self, camera=0, detectionCon=0.7, maxHands=2, cam_display=True, cam_height=480,
                                         cam_width=888):
        """
        This function is for hand detection configuration.
        Before using 'Jarvis_detect_hands' you need to call this function to setup things.
    ...
    def Jarvis_detect_hands(self, message=""):
        """
    ...
    def chatbot_base(self, input_text):
        ...
        Note:
            ...
            2. Set 'Jarvis.JarvisAssistant(high_accuracy_chatbot_model=False)' for low memory system (It gives low accuracy)
        @param input_text: str (example: Hello!)
        @return: str
        """
        if not self.load_chatbot_model:
            print("You need to Set 'Jarvis.JarvisAssistant(load_chatbot_model=True)' first")
    ...
    def chatbot_large(self, input_text):
        ...
        Note:
           ...
           2. Set 'Jarvis.JarvisAssistant(high_accuracy_chatbot_model=True)' for high memory system (It gives high accuracy)
        @param input_text: str (example: Hello!)
        @return: str
        """
        if not self.load_chatbot_model:
    ...
    def chatbot_advance(self, input_text, enable_google=False, enable_youtube=False):
        ...
        If chatbot can't answer then it will do wikipedia/google search
        Note:
            1. Set 'Jarvis.JarvisAssistant(load_chatbot_model=False)'
            2. Set 'Jarvis.JarvisAssistant(high_accuracy_chatbot_model=False)'
            3. Set 'Jarvis.JarvisAssistant(chatbot_large=False)'
            4. Set 'Jarvis.JarvisAssistant(high_accuracy_chatbot_model_with_neural_engine=True)'
        ...
        if not self.high_accuracy_chatbot_model_with_neural_engine:
            print(
                "You need to Set 'Jarvis.JarvisAssistant(high_accuracy_chatbot_model_with_neural_engine=True)'"
                " first to use 'chatbot_advance'")
            return None, False
        else:
            response = self.Jarvis_neural_engine_obj.predict_outcome(text=input_text, enable_google=enable_google,
...
if __name__ == '__main__':
    ...
    print(obj.show_me_my_list())
    print(obj.check_internet_speed())
    # obj.Jarvis_detect_hands()
